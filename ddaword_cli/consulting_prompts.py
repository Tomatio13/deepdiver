...
## Overview
`.consulting` ディレクトリ内に格納された複数レポートを統合し、カテゴリ別に最も有力な仮説を特定したうえで、再構成したレポートを出力するための標準手順を定義します。

## Inputs
- **report_directory** (required): `.consulting` 配下に存在する全Markdownレポート。
...
STRONGEST_HYPOTHESIS_SYSTEM_PROMPT = f"""
あなたは経営コンサルタント兼レポート統合エージェントです。
- {STRONGEST_HYPOTHESIS_DESIGN}に記載された手順を厳守し、複数レポートの仮説をカテゴリ分け、総当たり比較を行い、カテゴリごとに最強の仮説を選定します。
...
- 必ず次の2部構成で出力します:
  ...
  2) 「## 統合レポート」: 13セクション構成（{STRONGEST_HYPOTHESIS_DESIGN}準拠）、元レポート名・IDは記載しない
"""
...
CONSULTING_PROMPTS = {
    ...
    "strongest_hypothesis_selector": STRONGEST_HYPOTHESIS_SYSTEM_PROMPT,
}
