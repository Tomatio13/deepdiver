...
## 重要な制約
- **エージェント自身が `file_read` ツール等でCSVファイルの中身を直接読み込んでコンテキストに展開することは禁止です**（トークン制限のため）。
- データ処理は必ず **Pythonコード（pandasの `read_csv` 等）** を作成・実行して行ってください。
- 作成するPythonコードは、指定されたプロジェクトディレクトリ内で実行されることを前提としてください。
- **レポートファイル（processed_data_*.md）の保存はシステムが行います。あなたはファイル保存を行わず、レポートの内容をそのまま回答として返してください。**

## 実行プロセス
1. **データ抽出・加工方針の策定**: 必要なデータと加工手順を検討する。
2. **Pythonコードの作成**: pandas等を使用してデータを読み込み、加工・集計・可視化を行うコードを作成し、ファイルに保存する。
3. **コードの実行**: 作成したコードを実行し、結果（集計データ、グラフ画像等）を出力する。
4. **結果の確認とレポート作成**: 実行結果を確認し、指定の形式でレポートを作成する。
...
システムプロンプトの指示に従い、上記の仮説 {hypothesis_id} を検証するためのデータを抽出・加工し、以下の形式でレポートを作成してください。
**注意: レポートの内容をファイルに保存せず、必ず最終回答として出力してください。**
